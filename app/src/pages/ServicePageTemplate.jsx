...
import { Shield, Award, Heart, Zap, Wrench, CheckCircle, MapPin, CreditCard, Phone, Clock, Settings, Package, ThumbsUp } from 'lucide-react';
...
const brandConfigs = {
  ...
  siemens: {
    ...
    heroImage: 'https://images.unsplash.com/photo-1556911220-e15b29be8c8f?w=1200&q=80',
    serviceImage1: boschServiceImg1,
  ...
  profilo: {
    ...
    heroImage: 'https://images.unsplash.com/photo-1585338107529-13afc5f02586?w=1200&q=80',
    serviceImage1: boschServiceImg1,
...
const FeatureCard = ({ icon: Icon, title, color }) => {
  ...
  return (
    <div className="flex flex-col items-center p-6 bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-gray-100">
      <div className={`w-16 h-16 ${classes.bg} rounded-full flex items-center justify-center mb-4 shadow-lg`}>
        <Icon className="w-8 h-8 text-white" />
      </div>
      <h3 className="text-sm font-semibold text-gray-800 text-center leading-tight">{title}</h3>
    </div>
  );
};

const ServiceHighlightCard = ({ title, description, imageSrc, color, isDark = false }) => (
  <div className="relative overflow-hidden rounded-2xl shadow-2xl group h-80">
    ...
    />
    <div className={`absolute inset-0 ${isDark ? 'bg-gradient-to-t from-black via-black/70 to-black/40' : `bg-gradient-to-t from-${color}-900 via-${color}-800/70 to-${color}-700/40`}`}></div>
    
    <div className="relative h-full flex flex-col justify-end p-8">
      <h3 className="text-3xl font-bold text-white mb-3 leading-tight">{title}</h3>
      <p className="text-gray-100 text-base mb-6 leading-relaxed">{description}</p>
      
      <button className={`inline-flex items-center gap-2 bg-white text-${color}-600 px-6 py-3 rounded-lg font-semibold hover:bg-${color}-50 transition-all duration-300 shadow-lg hover:shadow-xl w-fit group`}>
...
const AdvantageCard = ({ icon: Icon, title, description, color }) => {
  ...
  return (
    <div className="bg-white rounded-xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100">
      <div className={`w-16 h-16 ${classes.lightBg} rounded-full flex items-center justify-center mb-6`}>
...
export default function ServicePageTemplate() {
  ...
  const location = locations.find(loc => loc.slug === selectedLocationSlug) || locations[0];
  ...
  const advantages = [
    ...
    },
    {
      ...
      description: 'Muğla Servisi olarak, beyaz eşyalarınıza uyguladığımız her işlem için 1 yıl garantili hizmet sunuyoruz.',
      color: color
    },
    {
      icon: MapPin,
      title: 'Muğla Geneli Servis',
      description: 'Muğla geneli hizmet ağı ile siz müşterilerimize kesintisiz hizmet vermekteyiz.',
      color: color
    },
    {
      icon: CreditCard,
      title: 'Kapıda Ödeyin',
      description: 'Ödemenizi kapıda ödeyerek, yerinde servis hizmetinden kolayca yararlanın.',
      color: color
    },
  ];

  const steps = [
    { title: 'Arıza Kaydı Oluşturalım', description: '0252 502 50 42 veya 0544 673 47 60 çağrı merkezimizi arayarak ürününüzle ilgili arıza kaydı oluşturun.' },
    { title: 'Teslim Alma & Teknik Servis', description: 'Arıza kaydı oluşturduğunuz ürününüzü dilediğiniz adresten alıyor ve işlemlere başlamak için teknik servis bölümümüze götürüyoruz.' },
    { title: 'Teslimat', description: 'Test edilerek teslimata hazır olan ürününüzü dilediğiniz adrese getirerek ve montajını yaparak teslim ediyoruz.' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
      ...
      {/* HERO BÖLÜMÜ */}
      <div className={`relative bg-gradient-to-br ${brand.colors.gradient} overflow-hidden`}>
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="flex items-center justify-center gap-2 mb-6">
            <div className="bg-white/20 backdrop-blur-sm rounded-full px-6 py-3 flex items-center gap-3 shadow-lg">
              ...
              <span className="text-white font-semibold text-lg">7/24 Çağrı Merkezi: 0252 502 50 42</span>
            </div>
            {/* 
 ...
 */}
          ...
          <p className="text-xl text-white/90 text-center max-w-3xl mx-auto leading-relaxed">
            Muğla ve ilçelerinde profesyonel, hızlı ve güvenilir teknik servis hizmeti
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12 mb-16 border border-gray-100">
          <h2 className="text-3xl font-bold text-gray-900 mb-6">{location.name} {brand.name} Servisi</h2>
          <div className="prose prose-lg max-w-none text-gray-700 leading-relaxed space-y-4">
            {serviceDescription.trim().split('\n\n').map((paragraph, idx) => (
        ...
        {/* Garanti Özellikleri */}
        <div className="mb-16">
          <h2 className="text-3xl font-bold text-center text-gray-900 mb-12">Hizmet Güvencelerimiz</h2>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {features.map((feature, index) => (
              <FeatureCard key={index} icon={feature.icon} title={feature.title} color={color} />
            ))}
          </div>
        </div>
        ...
        <div className="mb-16">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-600">Hizmet Kaliteli, Sorun Çözüm Odaklı</p>
          </div>
          <div className="grid md:grid-cols-2 gap-8">
            {advantages.map((adv, index) => (
              <AdvantageCard key={index} icon={adv.icon} title={adv.title} description={adv.description} color={adv.color} />
            ))}
          </div>
        </div>

        {/* 3 Adımda Çözüm */}
        <div className={`bg-gradient-to-br ${brand.colors.gradient} rounded-3xl shadow-2xl overflow-hidden`}>
          <div className="grid lg:grid-cols-2 gap-0">
            {/* Sol Kısım */}
            <div className={`bg-gradient-to-br from-${brand.colors.secondary} to-${brand.colors.accent} p-8 md:p-12 flex flex-col justify-center text-white`}>
              <div className="mb-8">
                <div className="inline-flex items-center gap-3 bg-white/20 backdrop-blur-sm rounded-full px-6 py-3 mb-6">
                  <Clock className="w-6 h-6" />
                  <span className="font-semibold text-lg">ÜCRETSİZ ÇAĞRI MERKEZİ</span>
                </div>
                <h3 className="text-3xl md:text-4xl font-bold mb-4 leading-tight">
                  MUĞLA VE İLÇELERİNE<br />7/24 SERVİS HİZMETİ
                </h3>
              </div>
              
              <div className="space-y-4">
                <a href="tel:4448166" className={`flex items-center gap-4 bg-white text-${brand.colors.primary} rounded-xl p-6 hover:bg-gray-50 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1`}>
                  <Phone className="w-8 h-8" />
                  <div>
                    <div className="text-sm font-medium opacity-75">Çağrı Merkezi</div>
                    <div className="text-2xl font-bold"> 0252 502 50 42 </div>
                  </div>
                </a>
                
                <a href="tel:02526921020" className="flex items-center gap-4 bg-white/10 backdrop-blur-sm border-2 border-white/30 text-white rounded-xl p-6 hover:bg-white/20 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1">
                  <Phone className="w-8 h-8" />
                  <div>
                    <div className="text-sm font-medium opacity-90">Alternatif Hat</div>
            ...
            <div className="bg-white p-8 md:p-12">
              <h3 className="text-3xl font-bold text-gray-900 mb-8">3 Adımda Çözüme Ulaşın</h3>
              <div className="space-y-6">
                {steps.map((step, index) => (
                  <div key={index} className="flex gap-4 group">
                    <div className="flex-shrink-0">
                      <div className={`w-12 h-12 bg-gradient-to-br from-${brand.colors.primary} to-${brand.colors.secondary} rounded-full flex items-center justify-center shadow-lg group-hover:shadow-xl transition-all duration-300`}>
